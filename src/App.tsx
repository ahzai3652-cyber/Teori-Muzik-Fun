...
export default function App() {
  ...
  // Track specific module progress for the certificate
  const [moduleProgress, setModuleProgress] = useState<Record<string, { score: number, total: number }>>({});
  
  // Accumulated score for general tracking (optional, but keeping it for now)
  ...
  return (
    <div className="min-h-screen bg-[#1a0b3b] text-white font-sans selection:bg-[#FFD700] selection:text-[#1a0b3b]">
      <AnimatePresence mode="wait">
        ...
        {screen === 'module1' && (
          <MisiNotasi 
            ...
            onComplete={(s, t) => {
              setModuleProgress(prev => ({ ...prev, module1: { score: s, total: t } }));
              setScore(score + s);
        ...
        {screen === 'module2' && (
          <CabaranIrama 
            onUpdateCoins={(c) => setCoins(prev => prev + c)}
            onComplete={(s, t) => {
              setModuleProgress(prev => ({ ...prev, module2: { score: s, total: t } }));
              setScore(score + s);
              setTotalQuestions(totalQuestions + t);
              setScreen('menu');
            }} 
        ...
        {screen === 'module3' && (
          <MasteriSimbol 
            onUpdateCoins={(c) => setCoins(prev => prev + c)}
            onComplete={(s, t) => {
              setModuleProgress(prev => ({ ...prev, module3: { score: s, total: t } }));
              setScore(score + s);
              setTotalQuestions(totalQuestions + t);
              setScreen('menu'); // Go back to menu instead of directly to result
            }} 
            onBack={() => setScreen('menu')}
          />
        )}
      ...
      {/* Audio Status & Coin Display */}
      {screen !== 'splash' && (
        <div className="fixed top-6 right-6 z-50 flex items-center gap-4">
          <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.2)]">
...
function MenuScreen({ onSelect, ownedInstruments, moduleProgress }: { onSelect: (m: Screen) => void, ownedInstruments: string[], moduleProgress: Record<string, { score: number, total: number }> }) {
  const modules = [
  ...
  const requiredModules = ['module1', 'module2', 'module3'];
  const completedRequired = requiredModules.filter(m => moduleProgress[m]);
  const isFinalEligible = completedRequired.length === requiredModules.length;
    ...
    >
      ...
      {isFinalEligible && (
        <motion.button
          ...
          className="mb-12 flex items-center gap-4 bg-gradient-to-r from-yellow-500 to-amber-600 px-8 py-4 rounded-full shadow-[0_0_30px_rgba(255,215,0,0.4)] hover:scale-105 transition-all text-[#1a0b3b] font-black italic tracking-tighter"
        >
          <Award className="w-8 h-8" />
          <span className="text-xl">LIHAT SIJIL PENCAPAIAN</span>
        </motion.button>
      )}
      ...
      <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-8 w-full max-w-7xl">
        {modules.map((m, i) => {
          ...
          const isDone = moduleProgress[m.id as string];
          return (
            <motion.button
              key={m.id}
              ...
              className={`group relative flex flex-col items-start p-8 rounded-3xl bg-white/5 border border-white/10 ${isLocked ? 'grayscale' : 'hover:border-[#FFD700] hover:bg-white/10'} backdrop-blur-xl transition-all text-left ${isDone ? 'border-green-500/50 bg-green-500/5' : ''}`}
            >
              ...
              <h3 className="text-2xl font-bold mb-2 group-hover:text-[#FFD700] transition-colors flex items-center gap-2">
                {m.title}
              ...
              <div className="mt-8 flex items-center text-sm font-bold text-[#FFD700] opacity-0 group-hover:opacity-100 transition-opacity">
                <span>{isLocked ? 'DIKUNCI' : isDone ? 'MAIN LAGI' : 'MULA SEKARANG'}</span>
...
function MisiNotasi({ onComplete, onUpdateCoins, onBack }: { onComplete: (s: number, t: number) => void, onUpdateCoins: (c: number) => void, onBack: () => void }) {
...
function CabaranIrama({ onComplete, onUpdateCoins, onBack }: { onComplete: (s: number, t: number) => void, onUpdateCoins: (c: number) => void, onBack: () => void }) {
  ...
  const handleChoice = (val: number) => {
    if (val === currentQ.value) {
      setGameScore(s => s + 1);
      onUpdateCoins && onUpdateCoins(10);
...
function MasteriSimbol({ onComplete, onUpdateCoins, onBack }: { onComplete: (s: number, t: number) => void, onUpdateCoins: (c: number) => void, onBack: () => void }) {
  const questions = useMemo(() => {
  ...
  const handleChoice = (val: string | number) => {
    if (val === currentQ.value) {
      setGameScore(s => s + 1);
      onUpdateCoins && onUpdateCoins(15);
      audioEngine.playSuccess();
      setShowFeedback(true);
    } else {
      audioEngine.playError();
...
function IramaKilat({ coins, ownedInstruments, onUpdateCoins, onBack }: { coins: number, ownedInstruments: string[], onUpdateCoins: (c: number) => void, onBack: () => void }) {
  ...
  const handleTap = (laneIdx: number) => {
    ...
    if (targetTile && targetTile.y > 60 && targetTile.y < 95) {
      ...
      // Speed up slightly
      setSpeed(prev => Math.min(prev + 0.015, 3.5));
    } else {
    ...
    const animate = (time: number) => {
      ...
      setTiles(prev => {
        ...
        if (missedCount > 0) {
          audioEngine.playError();
          setGameState('gameover');
          return []; // Clear tiles on game over to stop checks
        }
        return nextTiles;
      });
  ...
  return (
    ...
    >
      ...
      {gameState === 'lobby' && (
        <div className="text-center max-w-xl bg-white/5 backdrop-blur-xl p-8 rounded-3xl border border-white/10 mt-20">
          <Zap className="w-16 h-16 text-yellow-500 mx-auto mb-4" />
          <h2 className="text-4xl font-black mb-4 tracking-tighter">IRAMA KILAT</h2>
          <p className="text-purple-200/60 mb-12 font-medium">Uji kepantasan anda! Tap tile yang jatuh tepat pada waktunya.</p>
...
function KedaiMuzik({ coins, ownedInstruments, onBuy, onBack }: { 
  ...
  const shopItems = [
    ...
    { id: 'piano', label: 'PIANO', price: 1000, icon: <Piano className="w-12 h-12" />, color: 'bg-yellow-500' },
    { id: 'recorder', label: 'REKODER', price: 1500, icon: <Volume2 className="w-12 h-12" />, color: 'bg-blue-500' },
  ];
    ...
    >
        ...
        {shopItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              >
                {isOwned ? 'DIMILIK' : canAfford ? 'BELI SEKARANG' : 'WANG TIDAK CUKUP'}
              </button>
      ...
      <div className="mt-16 bg-white/5 backdrop-blur-md p-6 rounded-3xl border border-white/10 text-center max-w-lg">
        <p className="text-purple-200 text-sm italic">
          "Dapatkan mata wang dengan bermain **Irama Kilat**"
        </p>
      </div>
    </motion.div>
  );
...
function ResultScreen({ name, moduleProgress, onReset }: { name: string, moduleProgress: Record<string, { score: number, total: number }>, onReset: () => void }) {
  const modulesToCheck = ['module1', 'module2', 'module3'];
  ...
  modulesToCheck.forEach(m => {
    if (moduleProgress[m]) {
      totalScore += moduleProgress[m].score;
      totalPossible += moduleProgress[m].total;
    }
  });
  ...
  return (
    ...
    >
      <div className="max-w-4xl w-full text-center">
        ...
        <h2 className="text-5xl font-bold text-[#FFD700] mb-4 uppercase">KEPUTUSAN ANDA, {name.split(' ')[0]}!</h2>
        <p className="text-2xl text-purple-200 mb-12">
          {isEligibleForCert 
            ? 'Tahniah! Anda telah menamatkan cabaran dengan cemerlang.' 
            : 'Teruskan usaha! Jawab lagi untuk mendapatkan sijil pencapaian.'}
        </p>

        {/* Certificate Component - Only shown if eligible */}
        {isEligibleForCert ? (
          <div id="sijil-container" className="bg-white p-2 rounded-lg shadow-[0_0_50px_rgba(255,215,0,0.3)] mb-12 overflow-hidden">
            <div className="border-8 border-[#2D1B69] p-8 md:p-12 relative flex flex-col items-center bg-white text-[#1a0b3b]">
               ...
               <div className="grid grid-cols-3 gap-12 w-full">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-3xl font-black text-[#2D1B69]">{totalScore}/{totalPossible}</span>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[40px] p-12 mb-12 text-center">
            <Award className="w-24 h-24 text-white/20 mx-auto mb-6" />
            <h3 className="text-3xl font-bold mb-4">Sijil Belum Layak</h3>
            <p className="text-purple-200/60 max-w-md mx-auto mb-8">
              Anda perlu mencapai sekurang-kurangnya <span className="text-[#FFD700] font-bold">80%</span> untuk layak menerima sijil pencapaian digital.
            </p>
            <div className="flex justify-center gap-12">
              <div className="text-center">
                <p className="text-4xl font-black text-[#FFD700]">{percentage}%</p>
                <p className="text-xs uppercase tracking-widest opacity-40">Skor Anda</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-black text-red-400">80%</p>
                <p className="text-xs uppercase tracking-widest opacity-40">Sasaran</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-center">
          {isEligibleForCert && (
            <button 
              ...
              className="group flex items-center gap-3 px-10 py-5 bg-[#FFD700] text-[#1a0b3b] font-bold text-xl rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-yellow-500/20"
            >
              <Download className="w-6 h-6" />
              <span>MUAT TURUN SIJIL</span>
            </button>
          )}
