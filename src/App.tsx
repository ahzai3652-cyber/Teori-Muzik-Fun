...
export default function App() {
  ...
  const [ownedInstruments, setOwnedInstruments] = useState<string[]>(['piano']);
      ...
      {screen !== 'splash' && (
        <div className="fixed top-6 right-6 z-50 flex items-center gap-4">
          <button 
            ...
            className="flex items-center gap-2 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 hover:bg-white/60 transition-all text-[10px] font-bold tracking-widest text-white/60"
            title="Aktifkan Bunyi"
          >
            <Volume2 className="w-4 h-4" />
            <span>AKTIFKAN BUNYI</span>
          </button>
      ...
      {gameState === 'lobby' && (
        <div className="text-center max-w-xl bg-white/5 backdrop-blur-xl p-8 rounded-3xl border border-white/10 mt-20">
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {['piano', 'recorder', 'drum'].map(inst => {
              const isOwned = ownedInstruments.includes(inst) || inst === 'piano';
              return (
                <button
                  key={inst}
                  disabled={!isOwned}
                  onClick={() => setInstrument(inst as any)}
                  className={`py-4 rounded-xl font-bold flex flex-col items-center gap-2 transition-all border-2 ${
                    instrument === inst 
                ...
                >
                  {inst === 'piano' && <PianoIcon className="w-6 h-6" />}
                  {inst === 'recorder' && <Volume2 className="w-6 h-6" />}
                  {inst === 'drum' && <Music className="w-6 h-6" />}
                  <span className="text-[10px] uppercase tracking-widest">{inst === 'recorder' ? 'rekoder' : inst === 'drum' ? 'gendang' : inst}</span>
                  {!isOwned && <span className="text-[8px] text-red-500">BELUM DIMILIKI</span>}
                </button>
              );
            })}
          </div>
