...
class AudioEngine {
  ...
  async init() {
    ...
    try {
      ...
      // Improve scheduling stability and latency
      // Tone.context.latencyHint and lookAhead are often read-only on some environments
      ...
      this.masterVolume = new Tone.Volume(-5).connect(this.compressor);

      // PolySynth setup with explicit voice limits and simplified envelopes
      this.pianoSynth = new Tone.PolySynth(Tone.Synth, {
        oscillator: { type: 'sine' }, // Simpler oscillator
        envelope: { attack: 0.02, decay: 0.1, sustain: 0.3, release: 0.4 },
      }).connect(this.masterVolume);
      this.pianoSynth.maxPolyphony = 6;
      ...
      this.recorderSynth.maxPolyphony = 4;
  ...
  playNote(frequency: number, duration: string | number = "4n", type: 'piano' | 'recorder' | 'drum' = 'piano') {
    if (!this.initialized) {
      this.init().then(() => this.playNote(frequency, duration, type));
      return;
    }
  ...
  playPercussion(duration: string | number = "16n") {
    if (!this.initialized) {
      this.init().then(() => this.playPercussion(duration));
      return;
    }
  ...
  playSuccess() {
    if (!this.initialized) {
      this.init().then(() => this.playSuccess());
      return;
    }
